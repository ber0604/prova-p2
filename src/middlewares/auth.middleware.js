  ...
  return (req, res, next) => {
    ...
    if (newUserRole === 'admin' && loggedUserRole !== 'admin') {
      return res
        .status(403)
        .json({
          message: 'Apenas administradores podem criar usuários com role admin',
        });
    }
  ...
  return (req, res, next) => {
    ...
    if (userIdAtualizar != userIdLogado && req.user.role !== 'admin') {
      return res
        .status(403)
        .json({
          message: 'Apenas administradores podem editar outros usuarios',
        });
    }
...
module.exports = {
  authenticateToken,
  authorizeRole,
  authorizeUserRegister,
  authorizeUserEdition,
};
